...
describe('Test Fleet deployment on PRIVATE repos with HTTP auth', { tags: ['@p0', '@pr-tests'] }, () => {
  ...
  const repoTestData: testData[] = [
    ...
    // {qase_id: 8, provider: 'Bitbucket', repoUrl: 'https://bitbucket.org/fleetqa1/fleet-qa-examples.git'},
    {
      qase_id: 98,
      provider: 'Azure',
      repoUrl: 'https://dev.azure.com/fleetqateam/fleet-qa-examples/_git/fleet-qa-examples',
    },
  ];
  ...
  repoTestData.forEach(({ qase_id, provider, repoUrl }) => {
    it(
      qase(
        qase_id,
        `FLEET-${qase_id}: Test to install "NGINX" app using "HTTP" auth on "${provider}" PRIVATE repository`,
      ),
      { tags: `@fleet-${qase_id}`, retries: 1 },
      () => {
  ...
  if (!/\/2\.11/.test(Cypress.expose('rancher_version'))) {
    it(
      qase(191, 'FLEET-191: Test Fleet can be deployed in private Github repo using solely private token'),
      { tags: `@fleet-191` },
      () => {
...
describe('Test Fleet deployment on PRIVATE repos with SSH auth', { tags: '@p0' }, () => {
  ...
  const repoTestData: testData[] = [
    { qase_id: 2, provider: 'GitLab', repoUrl: 'git@gitlab.com:fleetqa/fleet-qa-examples.git' },
    {
      qase_id: 3,
      provider: 'Bitbucket',
      repoUrl: 'git@bitbucket.org:fleetqa-bb/fleet-qa-examples.git',
    },
  ...
  repoTestData.forEach(({ qase_id, provider, repoUrl }) => {
    it(
      qase(
        qase_id,
        `FLEET-${qase_id}: Test to install "NGINX" app using "SSH" auth on "${provider}" PRIVATE repository`,
      ),
      { tags: `@fleet-${qase_id}`, retries: 1 },
      () => {
        const repoName = `default-cluster-fleet-${qase_id}`;
...
describe('Test Fleet deployment on PRIVATE repos using KNOWN HOSTS', { tags: '@p0' }, () => {
  ...
  before('Preparing known hosts secrets via UI', () => {
    ...
    secretKnownHostsKeys.forEach((secretKnownHostsKeys) => {
      cy.clickButton('Create');
      cy.contains(new RegExp(sshString.join('|')))
        .should('be.visible')
        .click();
      cy.clickButton('Edit as YAML');
  ...
  // Custom / no default
  it(
    qase(141, 'FLEET-141  Test to install "NGINX" app using "KNOWN HOSTS" auth on PRIVATE repository'),
    { tags: '@fleet-141' },
    () => {
  ...
  it(
    qase(
      143,
      'FLEET-143  Test apps cannot be installed when using missmatched "KNOWN HOSTS" auth on PRIVATE repository',
    ),
    { tags: '@fleet-143' },
    () => {
  ...
  // Default verify
  it(qase(168, 'FLEET-168 Verify Fleet default known-host is set on configmap'), { tags: '@fleet-168' }, () => {
    // Create private repo using known host
  ...
  it(
    qase(
      169,
      'FLEET-169 Verify that without custom known-host, fleet uses default custom ones from defined in configmap',
    ),
    { tags: '@fleet-169' },
    () => {
  ...
  it(
    qase(170, 'FLEET-170 Verify that without ssh-key on private repo, custom known-host does not apply'),
    { tags: '@fleet-170' },
    () => {
  ...
  it(
    qase(
      171,
      'FLEET-171 Verify that without custom nor default known-host a gitrepo that needs this validation cannot be installed',
    ),
    { tags: '@fleet-171' },
    () => {
...
describe('Test gitrepos with cabundle', { tags: '@p0' }, () => {
  ...
  it(
    qase(144, 'Fleet-144 Test cabundle secrets are not created without TLS certificate'),
    { tags: '@fleet-144' },
    () => {
...
describe('Test Fleet with Webhook', { tags: ['@p0', '@pr-tests'] }, () => {
  ...
  it(qase(152, 'Fleet-152: Test Fleet with Webhook and disable polling '), { tags: '@fleet-152' }, () => {
    ...
    cy.exec('bash assets/webhook-tests/webhook_test_2_replicas.sh', {
      env: { gh_private_pwd },
    }).then((result) => {
      cy.log(result.stdout, result.stderr);
    });
  ...
  if (!/\/2\.11/.test(Cypress.expose('rancher_version'))) {
    ...
    it(
      qase(177, 'Fleet-177: Test Fleet with Webhook and secret using "gitjob-webhhook" on "cattle-fleet-system" '),
      { tags: '@fleet-177' },
      () => {
        const repoName = 'test-disable-polling';
...
describe('Test Fleet job cleanup', { tags: ['@p0', '@pr-tests'] }, () => {
  ...
  it(
    qase(148, 'Fleet-148: Test Fleet job clean-up with unsuccessful job is not deleted'),
    { tags: '@fleet-148' },
    () => {
...
describe('Test GitJob security context', { tags: ['@p0', '@pr-tests'] }, () => {
  it(qase(160, 'FLEET-160: Test GitJob pod security context'), { tags: '@fleet-160' }, () => {
    ...
    if (/\/2\.14/.test(Cypress.expose('rancher_version')) || /\/2\.15/.test(Cypress.expose('rancher_version'))) {
      ...
      cy.get('[data-testid="input-security-allowPrivilegeEscalation"] [role="checkbox"]').should(
        'have.attr',
        'aria-checked',
        'false',
      );
      ...
      cy.get('[data-testid="input-security-readOnlyRootFilesystem"] [role="checkbox"]').should(
        'have.attr',
        'aria-checked',
        'true',
      );
      ...
      // Check Run as Non-Root
      cy.get('input[name="runasNonRoot"]:checked').should('have.value', 'false');

      // Check Privilege Escalation
      cy.get('input[name="allowPrivilegeEscalation"]:checked').should('have.value', 'false');

      // Check Read Only Root File System
      cy.get('input[name="readOnlyRootFilesystem"]:checked').should('have.value', 'true');
    }
