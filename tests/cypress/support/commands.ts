...
Cypress.Commands.add('addPathOnGitRepoCreate', (path, index = 0) => {
  ...
  cy.get(
    `[data-testid="array-list-box${index}"] input[placeholder="e.g. /directory/in/your/repo"], [data-testid="array-list-box${index}"] input[placeholder="e.g. /directory/in/your/repo"]`,
  )
    .clear()
    .type(path);
});
...
Cypress.Commands.add('importYaml', ({ clusterName, yamlFilePath }) => {
  ...
  cy.get('[data-testid^="sortable-cell-"] .live-date')
    .each(($el) => {
...
Cypress.Commands.add(
  ...
  ({
    ...
    if (allowedTargetNamespace !== '') {
      cy.get('input[placeholder="Optional: Require all resources to be in this namespace"]').type(
        allowedTargetNamespace,
      );
    }
    ...
    if (tlsOption) {
      ...
      if ((tlsOption = 'Specify additional certificates')) {
        cy.readFile(tlsCertificate).then((content) => {
          cy.get(
            'textarea[placeholder="Paste in one or more certificates, starting with -----BEGIN CERTIFICATE----"]',
          ).type(content);
        });
      }
    }
...
Cypress.Commands.add('deployToClusterOrClusterGroup', (deployToTarget) => {
  cy.get(
    'div.labeled-select.create.hoverable, div.labeled-select.edit.hoverable, [data-testid="fleet-gitrepo-target-cluster"]',
  )
    .first()
    .should('be.visible');
  cy.get(
  ...
  if (checkNotInMenu === true) {
    cy.get('.list-unstyled.menu > li, [role="menuitem"]').each(($el) => {
      if ($el.text() != selection) {
        cy.log(
          `Cannot perform action with specified value "${selection}" since it is not present. Current Menu is: "${$el.text()}"`,
        );
    ...
    cy.get('.list-unstyled.menu > li > span, div.dropdownTarget', { timeout: 15000 })
      .contains(selection)
      .should('be.visible');
    cy.wait(500);
    cy.get('.list-unstyled.menu > li > span, div.dropdownTarget', { timeout: 15000 })
    ...
    if (selection === 'Force Update') {
      ...
      cy.get('body', { timeout: 10000 }).then(($body) => {
        if ($body.find('.card-title > h4').text().includes('Force Update')) {
          ...
          cy.get(buttonSelector).should('be.visible').and('contain.text', 'Update').click();
            ...
            if ($body.find(buttonSelector).length === 0) {
              ...
              cy.get(buttonSelector, { timeout: 30000 }).then(($btn) => {
                ...
                if (text === 'Update') {
                  ...
                  cy.get(buttonSelector, { timeout: 30000 }).should(($btn) => {
...
Cypress.Commands.add('verifyTableRow', (rowNumber, expectedText1, expectedText2, timeout = 60000) => {
  ...
  cy.contains('tr.main-row[data-testid="sortable-table-0-row"]').should('not.be.empty', {
    timeout: 25000,
  });
...
Cypress.Commands.add('accesMenuSelection', (firstAccessMenu = 'Continuous Delivery', secondAccessMenu, clickOption) => {
  ...
  if (clickOption) {
    ...
    cy.get('nav.side-nav')
      .contains(new RegExp('^' + clickOption + '$', 'g'))
      .should('be.visible')
      .click();
  }
...
Cypress.Commands.add('fleetNamespaceToggle', (toggleOption = 'local') => {
  cy.get('.vs__selected-options').contains('fleet-').click({ force: true });
    ...
    if ($body.text().match('/Actions/')) {
      ...
      cy.get('.btn')
        ...
        .then(() => {
          ...
          cy.contains('Delete').should('exist').click({ ctrlKey: true, force: true });
        });
...
Cypress.Commands.add(
  'checkGitRepoStatus',
  (repoName, bundles, resources, { timeout = 30000, repoStatus = 'Active' } = {}) => {
    ...
    if (bundles) {
      ...
      cy.get('div.fleet-status', { timeout: timeout })
        .eq(0)
        .contains(` ${bundles} Bundles ready `, { timeout: timeout })
        .should('be.visible');
    }
    ...
    if (resources) {
      ...
      cy.get('div.fleet-status', { timeout: timeout })
        .eq(1)
        .contains(` ${resources} Resources ready `, { timeout: timeout })
        .should('be.visible');
    } else {
...
Cypress.Commands.add(
  ...
  (
    appName,
    clusterName = 'local',
    appNamespace = 'Only User Namespaces',
    present = true,
    firstNav = 'Workloads',
    resourceToCheck = 'Pods',
  ) => {
...
Cypress.Commands.add('assignRoleToUser', (userName, roleName) => {
  ...
  cy.get(
    `span[aria-label='${roleName}'], div[class='checkbox-section checkbox-section--custom'] span[role='checkbox']`,
  ).scrollIntoView();
  cy.get(`span[aria-label='${roleName}'], div[class='checkbox-section checkbox-section--custom'] span[role='checkbox']`)
    .should('be.visible')
    .click();
...
Cypress.Commands.add('removeClusterLabels', (clusterName, key, value) => {
  ...
  cy.get('div.labels div.row div.key-value div.kv-container')
...
Cypress.Commands.add('gitRepoResourceCountAsInteger', (repoName, fleetNamespace = 'fleet-local') => {
  ...
  cy.get(
    "div[data-testid='gitrepo-deployment-summary'] div[class='count'], div[data-testid='resource-deployment-summary'] div[class=count]",
  )
    .invoke('text')
  ...
  if (uncheckStandardUser === true) {
    cy.get('body').then((body) => {
      if (body.find('span[aria-label="Standard User"]').length) {
        ...
        cy.get('span[aria-label="Standard User"]').should('be.visible').click();
...
Cypress.Commands.add('checkModalCardTitle', (expectedText, waitForRestart = true, shouldHaveText = true) => {
  ...
  cy.get('div.card-title').should('be.visible');
...
Cypress.Commands.add('moveClusterToWorkspace', (clusterName, workspaceName, timeout, restore = false) => {
  ...
  cy.contains(workspaceName, { matchCase: false }).should('be.visible').click({ force: true });
...
Cypress.Commands.add(
  ...
  (clusterName, timeout, defaultWorkspaceName = 'fleet-default', restore = true) => {
...
Cypress.Commands.add(
  ...
  (cloudProvider = 'Amazon', credentialName, accessKey, secretKey, region = 'eu-central-1') => {
