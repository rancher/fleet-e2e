...
Cypress.Commands.add('enableFeatureFlag', (flagName) => {
  ...
  .then(($el) => {
    if ($el.text().includes('Are you sure?')) {
      ...
      cy.checkModalCardTitle('Are you sure?', true, false);
  ...
  cy.checkModalCardTitle('Waiting for Restart', true)
})

Cypress.Commands.add('checkModalCardTitle', (expectedText, shouldHaveText, waitForRestart=true) => {
...
Cypress.Commands.add('moveClusterToWorkspace', (clusterName, workspaceName) => {
  ...
  cy.checkModalCardTitle('Assign Cluster To…', true, false);
  ...
  cy.clickNavMenu(['Clusters']);
  cy.filterInSearchBox(clusterName);
  cy.verifyTableRow(0, 'Wait Check-In', clusterName);
  cy.wait(30000);
  cy.verifyTableRow(0, 'Active', clusterName);
})
